/**
 * PDF Report Generator for SmartResume
...
export async function generatePDFReport(data, id) {
  ...
  doc.text('SmartResume', margin, 18);
  ...
  for (let p = 1; p <= totalPages; p++) {
    ...
    doc.setFontSize(7);
    setColor(doc, COLORS.textDim);
    doc.text('Generated by SmartResume AI - Powered by Ollama', pageWidth / 2, pageHeight - 11, { align: 'center' });
    doc.setFontSize(6);
  ...
  // SAVE
  // ═══════════════════════════════════════════════════════════════════════
  const pdfFilename = `SmartResume_Report_${id?.slice(0, 8) || 'analysis'}.pdf`;
