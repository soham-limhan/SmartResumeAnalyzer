/**
 * PDF Report Generator for ProfileX AI
...
export async function generatePDFReport(data, id) {
  ...
  doc.text('ProfileX AI', margin, 18);
  ...
  for (let p = 1; p <= totalPages; p++) {
    ...
    doc.setFontSize(7);
    setColor(doc, COLORS.textDim);
    doc.text('Generated by ProfileX AI AI - Powered by Ollama', pageWidth / 2, pageHeight - 11, { align: 'center' });
    doc.setFontSize(6);
  ...
  // SAVE
  // ═══════════════════════════════════════════════════════════════════════
  const pdfFilename = `ProfileX AI_Report_${id?.slice(0, 8) || 'analysis'}.pdf`;
